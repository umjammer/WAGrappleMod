...
public class GrappleLineRenderer extends EntityRenderer<GrappleLineEntity> {
    ...
    public GrappleLineRenderer(EntityRendererFactory.Context dispatcher) {
        super(dispatcher);
    }
    ...
    public void render(GrappleLineEntity entity, float yaw, float tickDelta, MatrixStack matrixStack,
        ...
        if (playerEntity != null) {
            // cause when you rotate or translate, you mutate the matrix.
            // but when you're done rendering, you need to restore the original state,
            // so as to not mess up everything else that renders after you (c)UpcraftLP
            matrixStack.push();
            matrixStack.push();
            ...
            if (itemStack.getItem() != WAGrappleMod.GRAPPLE_ITEM) {
                ...
                // neither of hands have an item - but we should draw it where it last was
                if (itemStack.getItem() != WAGrappleMod.GRAPPLE_ITEM) {
                    hand = prevHand;
                }
            }
            ...
            if ((this.dispatcher.gameOptions == null || this.dispatcher.gameOptions.getPerspective().ordinal() <= 0)
                ...
                fov /= 100.0D;
                vec3d = new Vec3d(hand * -0.37D * fov, -0.22D * fov, playerEntity.isSprinting()? 0.28 : 0.35D); // offset to the hand

                vec3d = vec3d.rotateX(-MathHelper.lerp(tickDelta, playerEntity.prevPitch, playerEntity.getPitch()) * 0.017453292F); // apply pitch
                vec3d = vec3d.rotateY(-MathHelper.lerp(tickDelta, playerEntity.prevYaw, playerEntity.getYaw()) * 0.017453292F);     // apply yaw
                ...
                nx = MathHelper.lerp(tickDelta, playerEntity.prevX, playerEntity.getX()) + vec3d.x; //player coordinates + offset 
                ny = MathHelper.lerp(tickDelta, playerEntity.prevY, playerEntity.getY()) + vec3d.y + playerEntity.getStandingEyeHeight();
                nz = MathHelper.lerp(tickDelta, playerEntity.prevZ, playerEntity.getZ()) + vec3d.z;
                lwidth = width;
            ...
            double x = entity.prevX == 0 ? entity.getX() : MathHelper.lerp(tickDelta, entity.prevX, entity.getX());// entity coordinates
            double y = entity.prevY == 0 ? entity.getY() : MathHelper.lerp(tickDelta, entity.prevY, entity.getY());
            double z = entity.prevZ == 0 ? entity.getZ() : MathHelper.lerp(tickDelta, entity.prevZ, entity.getZ());
            ...
            VertexConsumer consumer = vertexConsumerProvider
                    .getBuffer(RenderLayer.of("grapple_line",
                        ...
                        .layering(new RenderPhase.Layering("projection_layering", () -> {
                            ...
                            matrixStack2.push();
                            matrixStack2.scale(1.0F, 1.0F, 0.999F);
                        ...
                        .build(false)));  //creating custom render layer based off existing layers. adjusting values to fit the needs
            Matrix4f matrix4f2 = matrixStack.peek().getPositionMatrix(); // it didnt work without the matrix
